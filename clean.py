...
# Prompt: Load the raw scraped data produced by phase1_scrape.py
df = pd.read_csv('raw_data.csv')
 
# Prompt: Rename columns to clean snake_case names
df.columns = [
...
# Prompt: Convert ANNUAL_REV from mixed string formats (USD, $, M, mn, million)
# to a numeric float representing raw dollar value
...
# Prompt: Convert REV_GROWTH to numeric float, stripping % and + signs
...
# Prompt: Parse RD_SPEND — handle dollar/million formats AND "X% rev" entries
# by multiplying the percentage against the firm's annual revenue
...
# Prompt: Map AI_STATUS to binary — 1 = actively adopted, 0 = not adopted
# Drop ambiguous labels (In Review, N/A, Unknown, --) as they cannot be
# reliably classified as treatment or control
...
# Prompt: Convert TEAM_SIZE to numeric, handling K suffix (e.g., 1.2K)
...
# Prompt: Convert DIGITAL_SALES percentage string to numeric float
def parse_pct(val):
    if pd.isna(val) or str(val).strip() in ['', '--', 'N/A']:
        return np.nan
    try:
...
# Prompt: Convert CUSTOMER_ACCTS to numeric, handling K and M suffixes
...
# Prompt: Convert FOUNDED to integer year
df['FOUNDED'] = pd.to_numeric(df['FOUNDED'], errors='coerce')
 
# Prompt: Drop rows missing any critical variable before analysis
...
# Prompt: Save cleaned data to CSV for Phase 3 analysis
