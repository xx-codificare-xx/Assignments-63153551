...
# Prompt: Fetch the live page and raise an error if the request fails
response = requests.get(URL)
response.raise_for_status()
soup = BeautifulSoup(response.text, 'html.parser')

# Prompt: Locate the HTML table and extract column headers from the first row
table   = soup.find('table')
rows    = table.find_all('tr')
headers = [td.get_text(strip=True) for td in rows[0].find_all(['th', 'td'])]

# Prompt: Iterate over data rows — extract firm name from <strong>
...
# Prompt: Save raw scraped data to CSV for Phase 2 cleaning
